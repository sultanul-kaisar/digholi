                        ...
                        <div class="col-md-12 col-xl-12">
                            <div class="card sale-card" style="min-height:550px!important;">
                                <div class="card-header">
                                    ...
                                    @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('slider create'))
                                        <div class="card-header-right col-md-9">
                                            <a type="button" href="{{ route('slider.create') }}" class="btn btn-round waves-effect waves-light btn-primary btn-outline-primary float-right"><i class="icofont icofont-user-alt-3"></i> New Slider</a>
                                        </div>
                                    @endif
                                </div>
                                        ...
                                        <div class="table-responsive dt-responsive">
                                            <table id="multi-colum-dt" class="table table-striped table-bordered nowrap">
                                                ...
                                                <div class="content" >
                                                    <thead>
                                                    <tr>
                                                        @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('slider view') || auth()->user()->can('slider edit') || auth()->user()->can('slider delete'))
                                                            <th>Action</th>
                                                        @endif
                                                        <th>Title</th>
                                                        <th>Image</th>
                                                    ...
                                                    @foreach($sliders as $slider)
                                                        <tr>
                                                                    ...
                                                                    <div class="list-icons">
                                                                        <div class="dropdown">
                                                                            ...
                                                                            <div class="dropdown-menu dropdown-menu-right custom-dropdown-menu-right">
                                                                                @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('slider view'))
                                                                                    <a href="#" class="dropdown-item" onclick="event.preventDefault();getClient('{{ $slider->id }}', '{{ $slider->title }}', {{ $slider->title2 }}, {{ $slider->title3 }}, {{ $slider->body }}, '{{ $slider->image }}', '{{ asset('storage/uploads/sliders/') }}', '{{ $slider->align }}', '{{ $slider->status }}');"><i class="ti-pencil-alt"></i> View</a>
                                                                                @endif

                                                                                @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('slider edit'))
                                                                                    <a href="{{ route('slider.edit', $slider->id) }}" class="dropdown-item"><i class="ti-pencil-alt"></i> Edit</a>
                                                                                @endif

                                                                                @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('slider delete'))
                                                            ...
                                                            <td>{!! $slider->title !!}</td>
                                                            <td style="width:15%;">
                                                                <a href="{{ asset('storage/uploads/sliders/'.$slider->image) }}" data-toggle="lightbox">
                                                                    <img src="{{ asset('storage/uploads/sliders/'.$slider->image) }}" class="img-fluid">
                                                                </a>
                                                            </td>
                                                            <td>
                                                                @if($slider->align == "left")
                                                                    <span class="badge badge-info">Left</span>
                                                                @endif

                                                                @if($slider->align == "right")
                                                                    <span class="badge badge-dark">Right</span>
                                                                @endif
                                                            </td>
                                                            <td>
...
@push('js')
    ...
    <script>
        function getSlider(id, title, image, image_path ,align, status)
        {
            if(status == "left"){
                status = "<span class='badge badge-success' style='font-size:55%!important;padding: .2125rem .375rem!important;'>Left</span>"
            }

            if(status == "right"){
                status = "<span class='badge badge-danger' style='font-size:55%!important;padding: .2125rem .375rem!important;'>Right</span>"
            }
            ...
            document.getElementById("slider-align").innerHTML   = align;
