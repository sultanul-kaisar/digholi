                        ...
                        <div class="col-md-12 col-xl-12">
                            <div class="card sale-card" style="min-height:550px!important;">
                                <div class="card-header">
                                    <h5 class="card-title">All Gallery Categories</h5>
                                    @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('gallery category create'))
                                        <div class="card-header-right col-md-9">
                                            <a type="button" href="{{ route('gallery-category.create') }}" class="btn btn-round waves-effect waves-light btn-primary btn-outline-primary float-right"><i class="icofont icofont-plus-circle"></i> New Gallery Category</a>
                                        </div>
                                    @endif
                                </div>
                                    ...
                                    <div class="card-block">
                                        <div class="table-responsive dt-responsive">
                                            <table id="multi-colum-dt" class="table table-striped table-bordered nowrap">
                                                <thead>
                                                <tr>
                                                    @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('gallery category edit') || auth()->user()->can('gallery category delete'))
                                                        <th>Action</th>
                                                    @endif
                                                    <th>Title</th>
                                                    <th>Slug</th>
                                                ...
                                                @foreach($galleryCategories as $galleryCategory)
                                                    <tr>
                                                                ...
                                                                <div class="list-icons">
                                                                    <div class="dropdown">
                                                                        ...
                                                                        <div class="dropdown-menu dropdown-menu-right custom-dropdown-menu-right">
                                                                            @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('gallery category edit'))
                                                                                <a href="{{ route('gallery-category.edit', $galleryCategory->id) }}" class="dropdown-item"><i class="ti-pencil-alt"></i> View / Edit</a>
                                                                            @endif

                                                                            @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('gallery category delete'))
                                                                                <a href="{{ route('admin.dashboard') }}" class="dropdown-item" onclick="event.preventDefault();document.getElementById('delete-form{{$galleryCategory->id}}').submit();"><i class="ti-trash"></i> Delete</a>
                                                                                <form id="delete-form{{$galleryCategory->id}}" action="{{ route('gallery-category.destroy', $galleryCategory->id) }}" method="POST" style="display: none;">
                                                                                    @csrf
                                                                                    @method('DELETE')
                                                                                </form>
                                                                            @endif
                                                        ...
                                                        <td>
                                                            @if(!is_null($galleryCategory->parent))
                                                                {{ $galleryCategory->parent->title }}
                                                            @else
                                                                --
                                                            @endif
                                                        </td>
                                                        <td>
                                                            @if($galleryCategory->status == "active")
                                                                <span class="badge badge-success">Enabled</span>
                                                            @endif

                                                            @if($galleryCategory->status == "inactive")
