            ...
            <div class="col-lg-0">
                <div class="page-header-breadcrumb">
                    <ul class=" breadcrumb breadcrumb-title">
                        ...
                        <li class="breadcrumb-item"><a href="#!"></a> Blog Categories</li>
                    </ul>
                </div>
            </div>
        </div>
                                ...
                                <div class="card-header">
                                    ...
                                    @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('blog category create'))
                                        <div class="card-header-right col-md-9">
                                            <a type="button" href="{{ route('blog-category.create') }}" class="btn btn-round waves-effect waves-light btn-primary btn-outline-primary float-right"><i class="icofont icofont-plus-circle"></i> New Blog Category</a>
                                        </div>
                                    @endif
                                </div>
                                    ...
                                    <div class="card-block">
                                        <div class="table-responsive dt-responsive">
                                            <table id="multi-colum-dt" class="table table-striped table-bordered nowrap">
                                                ...
                                                <thead>
                                                <tr>
                                                    @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('blog category edit') || auth()->user()->can('blog category delete'))
                                                        <th>Action</th>
                                                    @endif
                                                    <th>Title</th>
                                                    <th>Slug</th>
                                                ...
                                                @foreach($blogCategories as $blogCategory)
                                                    <tr>
                                                                ...
                                                                <div class="list-icons">
                                                                    <div class="dropdown">
                                                                        ...
                                                                        <div class="dropdown-menu dropdown-menu-right custom-dropdown-menu-right">
                                                                            @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('blog category edit'))
                                                                                <a href="{{ route('blog-category.edit', $blogCategory->id) }}" class="dropdown-item"><i class="ti-pencil-alt"></i> View / Edit</a>
                                                                            @endif

                                                                            @if(auth()->user()->can('master') || auth()->user()->can('global') || auth()->user()->can('blog category delete'))
                                                                                <a href="{{ route('admin.dashboard') }}" class="dropdown-item" onclick="event.preventDefault();document.getElementById('delete-form{{$blogCategory->id}}').submit();"><i class="ti-trash"></i> Delete</a>
                                                                                <form id="delete-form{{$blogCategory->id}}" action="{{ route('blog-category.destroy', $blogCategory->id) }}" method="POST" style="display: none;">
                                                                                    @csrf
                                                                                    @method('DELETE')
                                                                                </form>
                                                                            @endif
                                                        ...
                                                        <td>
                                                            ...
                                                            @if($blogCategory->status == "inactive")
